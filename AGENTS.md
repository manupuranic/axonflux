...
- AI-generated highlight copy via Codex Haiku (`POST /{id}/ai/highlights`)
...
- Campaign editing UI: Edit Campaign dialog (title/type/objective/status/valid_until) + design Rename, both previously missing despite backend support; AI Design Chat renders real markdown (tables/lists/bold) and its panel is drag-resizable
- Theme engine hardening: sub-agent tool calls (set_theme's AI color generator) now inherit the caller's provider/model instead of guessing a hardcoded one (was silently defaulting to Anthropic direct + a since-verified-dead OpenRouter model slug); system prompts require checking tool results for errors before narrating success; `set_theme(clear_node_overrides=true)` fixes "reapply theme on the whole page" no-oping on nodes styled individually earlier (inline `style_overrides` always beat theme CSS variables); `style_region` now supports real color (not just font-size) on the `product_cards` region
...
Script: `scripts/generate_product_content.py` — takes barcode list → calls Codex API →
...
**C4 — RAG Chatbot**
Query → retrieve relevant products/signals/recommendations → Codex API → response.
...
| **Content Writer Agent** | Sequential | Topic seed → Codex draft (800–1200 words) → SEO pass → `app.blog_posts` (status: draft) → staff approves → static export |
...
- **Content Agent** — Codex API → description, tags, use_cases, key_benefits (wellness framing, no medical claims)
- **SEO Agent** — Codex API → seo_title (60 chars), seo_description (155 chars), schema.org JSON-LD, storefront_slug
...
- Draft: Codex API → 800–1200 words, wellness/utility tone, 2–3 internal product page links
