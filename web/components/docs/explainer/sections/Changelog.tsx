...
const ENTRIES: Entry[] = [
  ...
  {
    date: "2026-08-01",
    title: "AI chat theme engine — provider/model bug, dead model slug, false-success narration, inline-style-vs-theme trap",
    detail: "One user report ('theme didn't apply') unraveled into four stacked issues across the shared pamphlet/campaign-studio DSL tools. (1) set_theme's AI color generator spun up its own sub-session hardcoded to a per-provider 'cheap model' guess instead of the caller's already-working provider+model — silently called Anthropic direct even when the user was on OpenRouter, surfacing as an auth error. Fixed by threading provider/model through PamphletState from both call sites (campaign_studio/chat.py, pamphlets/ai_session.py) so sub-sessions inherit the real config. (2) Once routed correctly, the hardcoded OpenRouter 'cheap' slug (a dated Haiku ID) turned out not to exist on OpenRouter at all — verified live against openrouter.ai/api/v1/models rather than trusting the existing constant; corrected to the actually-cheapest, tool-capable, verified-real anthropic/claude-sonnet-5 everywhere the wrong slug appeared (image_agent.py, config.py suggestion lists, ai-settings.ts, router.py labels). (3) Both system prompts told the model to 'write one confirmation sentence after tools complete' with no failure branch, so it narrated success even when a tool returned an error — added an explicit check-every-result-for-error-first rule. (4) 'Reapply the theme on the whole page' silently no-op'd on any node styled individually earlier in the conversation, because node-level style_overrides render as inline styles that always beat theme CSS variables — confirmed by pulling the live DSL from Postgres and finding pinned hex colors on specific node IDs. Added set_theme(clear_node_overrides=true) to strip color-only overrides before reapplying, wired into both system prompts for whole-page requests only. Also shipped real color support for the product_cards region (badge/price color via new --card-badge/--card-price CSS vars) since style_region previously rejected any color request there outright.",
    type: "fix",
  },
  {
    date: "2026-07-26",
