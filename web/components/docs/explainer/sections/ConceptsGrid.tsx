...
const CONCEPTS: Concept[] = [
  ...
  },
  {
    ...
    definition: "Canvas dimensions live in the document's root node, not in the renderer. Every layer reads format from the document — never from hardcoded constants.",
    analogy: "Paper size belongs in print settings, not in printer firmware.",
    here: "Campaign Studio DSL: page.width_mm/height_mm. HTML renderer, PDF exporter, preview iframe all derive viewport from it. WhatsApp Square (1080×1080) or Story (1080×1920) = one DSL field; zero renderer code.",
    pitch: "When format is a constant in code, every new format is a deployment. When it's data in the document, the renderer becomes format-agnostic by construction — new formats are config, not code.",
    accent: "from-slate-50 to-gray-50 border-slate-200",
  },
  {
    emoji: "🗂️",
    title: "StaticFiles as dev-time storage",
    definition: "Uploaded files land on disk; FastAPI StaticFiles serves them. URL structure mirrors a real CDN — production swap is two env vars.",
    analogy: "A sticky note on your desk — same information, no cloud account required.",
    here: "POST /campaigns/{id}/assets/upload → data/uploads/campaign_assets/. app.mount('/uploads', StaticFiles(…)). Frontend uses ${API_BASE}/uploads/… — same URL shape as R2 or S3. Swap = two env vars, zero frontend code.",
    pitch: "Don't stand up MinIO or real S3 in local dev. A folder + StaticFiles is your object storage until you need CDN characteristics. Design the URL surface as a CDN from day one — the swap costs nothing.",
