...
const PHASES: {
...
}[] = [
  {
    ...
    items: [
      ...
      { name: "A4 · Role-based access control", detail: "Lattice staff(1) < manager(2) < admin(3) via a require_role() factory over ROLE_LEVELS — adding a role is one dict line. Unknown/missing role claims map to level 0 and fail every gate (previously they defaulted to staff). Migration 013 pins the users.role CHECK constraint. Endpoint audit: pipeline trigger/cancel manager+, pipeline reads staff+, cash verify manager+, entity confirm/reject manager+, recompute + alias delete admin, everything else require_staff. Admin user-management API (/api/users) + Users section on Settings. Frontend policy in one table (lib/permissions.ts) + minRole on sidebar links — UX only; server 403 is the boundary. Machine identities deliberately excluded: Phase E MCP gets a separate api_key plane.", status: "shipped" },
      ...
      { name: "A6 · Item Master Cleanup", detail: "Reusable ER4U workbook workflow: upload creates an independent immutable run; deterministic Name/Brand/Size/PACKED analysis and bounded semantic advisory checks run through the normal API; exact-baseline historical decisions are reused while stale decisions are invalidated; only new, changed, or ambiguous items enter the card-based Decision Inbox; Accept/Reject/Edit feed a fidelity-validated Excel export. Closed production proof: 13,507 source rows preserved, 226/226 naming items reviewed, zero protected-field changes.", status: "shipped" },
    ],
  },
  {
    key: "B",
